...
interface ProfessionalDAO {
    @Insert(onConflict = OnConflictStrategy.REPLACE) // Se já existir um item com a mesma chave, será substituído pelo novo item devido à estratégia de conflito REPLACE.
    ...
    @Query("SELECT * FROM professional WHERE email = :email")
    suspend fun getProfessionalByEmail(email: String): ProfessionalEntity?
